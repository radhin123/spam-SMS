...
def bingung():
	...
	if(pilih=="y"):
		...
		exit("Terima kasih ya bre!")
	else:
		print("Yang bener coi masukin nya!")
		bingung()
def cok():
	while True:
		try:
			...
			if a in(""," "):print("[!] Jangan Kosong bre")
				...
				try:
					suu=int(input("[+] Lu mau nyepam berapa bre\t: "))
				except:
					print("Masukan Format Angka Jangan Huruf!")
					cok()
				time.sleep(2)
				print("\n[+] Sedang Melakukan spam silahkandi tunggu usahakan koneksi internrt anda gak jelek V")
				b=a[1:12] 
				c="62"+b
...
def F()):
	print(f"{p}"+title+f"{h}\Power By Radhin Al Haady_ | 16 Operator Otp{p}\n\nJangan Disalah Gunakan Bro angan lupa minta maaf bre kalo udah nyepam oke\nJumlah Spam Dikalikan Dengan 16\nContoh Jumlah Spam 2\nJadinya 2 Dikali 16 Spamnya 32\n")
